...
export default class Search {
  ...
  async sendRequest() {
    try {
      // Hacer la solicitud GET a la ruta con el valor del campo input
      const results = await axios.get(`/search/${this.inputField.value}`);
      
      // Procesar los resultados de posts y users
      this.renderResultsHTML(results.data.posts, results.data.users);
    } catch (error) {
    ...
    if (posts.length || users.length) {
      this.resultsArea.innerHTML = DOMPurify.sanitize(`
        <div class="list-group shadow-sm">
          ...
          ${users.length ? `<div class="list-group-item"><strong>Users</strong> (${users.length})</div>` : ''}
          ${users
            .map(user => {
              return `<a href="/user/${user.id}" class="list-group-item list-group-item-action">
          ...
          <!-- Resultados de posts -->
          ${posts.length ? `<div class="list-group-item"><strong>Posts</strong> (${posts.length})</div>` : ''}
          ${posts
            .map(post => {
